...
use widow_lib::{lexer, parser, interpreter};
use widow_lib::memory::MemoryManager;

#[derive(Parser)]
#[command(name = "widow")]
#[command(about = "The Widow Programming Language")]
    ...
    match cli.command {
        ...
        None => {
            if let Some(file) = cli.file {
                ...
                eprintln!("{} No file specified. Use --help for usage information.", "Error:".bright_red());
                process::exit(1);
            }
        }
    }
...
fn run_file(file_path: PathBuf, verbose: bool) {
    if verbose {
        println!("{} Running Widow file: {}", "Info:".bright_blue(), file_path.display());
    }
    
    if !file_path.exists() {
        eprintln!("{} File not found: {}", "Error:".bright_red(), file_path.display());
        process::exit(1);
    }
...
fn compile_to_bytecode(file_path: PathBuf, output: Option<PathBuf>) {
    eprintln!("{} Bytecode compilation not yet implemented", "Error:".bright_red());
    process::exit(1);
}

fn execute_bytecode(file_path: PathBuf) {
    eprintln!("{} Bytecode execution not yet implemented", "Error:".bright_red());
    process::exit(1);
}

fn compile_to_native(file_path: PathBuf, output: Option<PathBuf>) {
    eprintln!("{} Native compilation not yet implemented", "Error:".bright_red());
    process::exit(1);
}
